...
function Project({ p_txt }: ProjectProps) {
  ...
  const handleClick = () => {
    window.location.href = p_txt.source;
  };

  return (
    <button
      onClick={handleClick}
      className="projectCard flex flex-col group text-start relative border-2 rounded-2xl transition-all duration-500 ease-in-out cursor-pointer"
    >
      ...
      <article className="bg-[var(--bg-secondary)] h-full rounded-[inherit]">
        ...
        <div className="flex flex-col text-sm md:text-base justify-start py-2 px-4">
