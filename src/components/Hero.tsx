...
export default function Hero() {
  ...
  return (
    <section className="heroSection grid grid-cols-1 gap-32 md:gap-0 md:grid-cols-2 justify-center text-[var(--text-primary)] py-20 md:py-28">
      ...
      <div className="links md:col-span-2 flex justify-center gap-4 md:mt-20 text-xl ">
        <button className="text-[var(--highlight-secondary)] border py-1 px-3 rounded-full hover:text-[var(--bg-primary)] hover:bg-[var(--highlight-secondary)] transition-colors duration-250">
          {txt.hero.cv}
        </button>
        <button
          onClick={() => handleRedirect("https://github.com/Pedroguibas")}
          className="p-2 bg-[var(--bg-secondary)] rounded-full hover:bg-[var(--highlight-primary)] hover:translate-y-[-2px] transition-all duration-200"
        >
          <Github />
        </button>
        <button
